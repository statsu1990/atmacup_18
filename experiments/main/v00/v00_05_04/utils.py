...
class CnnModel:
    ...
    def fit(
        ...
        tr_ds = AtmaCup18Dataset(
            tr_imgs, tr_features, tr_targets, image_transforms=[self.img_scaler]
        )
        vl_ds = AtmaCup18Dataset(
            vl_imgs, vl_features, vl_targets, image_transforms=[self.img_scaler]
        )
    ...
    def predict(self, images: np.ndarray, features: pl.DataFrame) -> np.ndarray:
        ...
        # dataset
        ds = AtmaCup18Dataset(
            images, features, targets=None, image_transforms=[self.img_scaler]
        )
        dl = _get_dataloader(
            ds,
            batch_size=self.fit_params["dnn"]["vl_batch_size"],
...
class DNN(nn.Module):
    def __init__(
        ...
        # head
        self.head = Head(
            in_channels=self.last_decoder_channels,
    ...
    def forward(self, x: torch.Tensor, features: torch.Tensor) -> torch.Tensor:
        ...
        """
        # backbone
        h, h_feat = self.backbone(x, features)
