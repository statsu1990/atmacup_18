...
class FeatureGBDT:
    ...
    def fit(
        ...
        optical_flow_images: np.ndarray,
    ):
        """
        Args:
            df (pl.DataFrame): 特徴量データ (n_sample, n_features)
            depth_images (np.ndarray): 深度画像データ (n_sample, image_name, height, width, 1)
            optical_flow_images (np.ndarray): 光学フロー画像データ (n_sample, image_name, height, width, channel)
        """
        # depthのpca
    ...
    def transform(
        ...
        # ##############
        df = df.with_columns(
            self._calc_optical_flow_patch_features(optical_flow_images),
        )
    ...
    def _calc_optical_flow_patch_features(
        self, optical_flow_images: np.array
    ) -> pl.DataFrame:
        """
        optical_flow画像データから特徴量を計算する

        Args:
            optical_flow_images (np.array): optical_flow画像データ (n_sample, image_name, height, width, channel)
        """
        pf = self.prefix

        # (n_sample, height, width, image_name * channel)
        optical_flow_images = optical_flow_images.transpose(0, 2, 3, 1, 4).reshape(
            optical_flow_images.shape[0],
            optical_flow_images.shape[2],
            optical_flow_images.shape[3],
            -1,
        )
        n_path_h = 4
        n_path_w = 4
        # (n_sample, n_patch, patch_h, patch_w, image_name * channel)
        optical_flow_images = self._image_to_patch(
            optical_flow_images, n_path_h, n_path_w
        )

        # (n_sample, n_patch, patch_h * patch_w, channel)
        optical_flow_images = optical_flow_images.reshape(
            optical_flow_images.shape[0],
            optical_flow_images.shape[1],
            -1,
            optical_flow_images.shape[4],
        )

        features_ = {}
        features_[f"{pf}optical_flow_patch_mean"] = optical_flow_images.mean(axis=2)
        features_[f"{pf}optical_flow_patch_std"] = optical_flow_images.std(axis=2)
        features_[f"{pf}optical_flow_patch_max"] = optical_flow_images.max(axis=2)
        features_[f"{pf}optical_flow_patch_min"] = optical_flow_images.min(axis=2)
        ...
        for ft_name, ft in features_.items():
            # ft: (n_sample, n_patch, channel)
            for i_patch in range(ft.shape[1]):
                for i_channel in range(ft.shape[2]):
                    # (n_sample,)
                    features[f"{ft_name}_p{i_patch}-c{i_channel}"] = ft[
                        :, i_patch, i_channel
                    ]
...
class XgbModel:
    ...
    def fit(
        ...
        tr_dmatrix = xgb.DMatrix(tr_features, label=tr_targets)
        vl_dmatrix = xgb.DMatrix(vl_features, label=vl_targets)

        evals = [(tr_dmatrix, "train"), (vl_dmatrix, "valid")]
        num_boost_round = self.fit_params["gbdt"]["num_boost_round"]
        early_stopping_rounds = self.fit_params["gbdt"]["early_stopping_rounds"]
    ...
    def predict(
        ...
        """
        # (n_sample, n_targets)
        preds = self.model.predict(xgb.DMatrix(features))
        return preds
