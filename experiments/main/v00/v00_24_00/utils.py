...
class BackBone(nn.Module):
    def __init__(
        ...
        self.img_encoder_config = Dinov2Config.from_pretrained(image_encoder_name)
        self.img_encoder = Dinov2Model.from_pretrained(image_encoder_name)
        self.resize_for_img_encoder = nn.Upsample(
            size=(
                self.img_encoder_config.image_size,
                self.img_encoder_config.image_size,
            ),
            mode="bilinear",
            align_corners=True,
        ...
        encoder_last_channels = self.img_encoder_config.hidden_size

        # feature
        self.ft_fc = nn.Sequential(
            nn.Linear(n_features // self.n_sample_in_scene + 1, encoder_last_channels),
        )

        # agg
        self.ft_agg_fc = nn.Sequential(
            nn.GELU(),
            nn.Linear(n_sample_in_scene * encoder_last_channels, embed_dim),
        )
    ...
    def forward(
        ...
        # encoder
        h = self.resize_for_img_encoder(h)
        # (batch * n_sample_in_scene, tokens, channels)
        h = self.img_encoder(pixel_values=h).last_hidden_state
        ...
        h = (
            h.view(n_batch, self.n_sample_in_scene, -1, h.size(-1))
            .permute(0, 2, 1, 3)
            .reshape(n_batch, -1, self.n_sample_in_scene * h.size(-1))
        )
