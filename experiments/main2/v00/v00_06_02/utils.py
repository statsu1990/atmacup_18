...
class FeatureGBDT:
    ...
    def transform(self, df: pl.DataFrame, depth_images: np.ndarray):
        ...
        df = df.with_columns(
            self._calc_depth_features(depth_images),
        )
    ...
    def _calc_depth_features(self, depth_patch: np.array) -> pl.DataFrame:
        """
        深度画像データから特徴量を計算する

        Args:
            depth_patch (np.array): 深度画像データ (n_sample, n_patch, patch_h, patch_w, channel)
        """
        pf = self.prefix

        # (n_sample, n_patch, patch_h * patch_w, channel)
        depth_patch = depth_patch.reshape(
            depth_patch.shape[0], depth_patch.shape[1], -1, depth_patch.shape[4]
        )

        features_ = {}
        features_[f"{pf}depth_patch_mean"] = depth_patch.mean(axis=2)
        features_[f"{pf}depth_patch_std"] = depth_patch.std(axis=2)
        features_[f"{pf}depth_patch_max"] = depth_patch.max(axis=2)
        features_[f"{pf}depth_patch_min"] = depth_patch.min(axis=2)
