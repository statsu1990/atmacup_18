...
# dnn architecture
def _change_conv_in_channels(conv: nn.Conv2d, new_in_channels: int):
    """
    conv2d の in_channels を変更する
    https://github.com/qubvel-org/segmentation_models.pytorch/blob/main/segmentation_models_pytorch/encoders/_utils.py#L5

    Args:
        conv (nn.Conv2d): 変更対象の Conv2d
        new_in_channels (int): 新しい in_channels
    """
    weight = conv.weight.detach()
    old_in_channels = conv.in_channels
    conv.in_channels = new_in_channels

    if new_in_channels == 1:
        new_weight = weight.sum(dim=1, keepdim=True)
        conv.weight = nn.parameter.Parameter(new_weight)
    else:
        new_weight = torch.Tensor(
            conv.out_channels, new_in_channels // conv.groups, *conv.kernel_size
        )

        for i in range(new_in_channels):
            new_weight[:, i] = weight[:, i % old_in_channels]

        new_weight = new_weight * (old_in_channels / new_in_channels)
        conv.weight = nn.parameter.Parameter(new_weight)


def _change_segmentation_mit_encoder_in_channels(
    model: nn.Module, new_in_channels: int
):
    """
    segmentation_models_pytorch の mit encoder の in_channels を変更する

    Args:
        model (nn.Module): 変更対象のモデル。segmentation_model_pytorchのUnetなど
        new_in_channels (int): 新しい in_channels
    """
    _change_conv_in_channels(model.encoder.patch_embed1.proj, new_in_channels)


def _get_segmentation_model(model_name: str, params: dict) -> nn.Module:
    """
    segmentation model を取得する
    ...
    if model_name == "unet":
        ...
        model_class = smp.MAnet
    elif model_name == "pan":
        model_class = smp.PAN
    elif model_name == "upernet":
        model_class = smp.UPerNet
    else:
    ...
    if encoder_name in ["mit_b0", "mit_b1", "mit_b2", "mit_b3", "mit_b4", "mit_b5"]:
        # in_channels=3のみにデフォルトでは対応しているため。自前で変更する
        in_channels = params["in_channels"]
        params["in_channels"] = 3
        model = model_class(**params)
        _change_segmentation_mit_encoder_in_channels(model, in_channels)
        return model
    else:
...
class BackBone(nn.Module):
    def __init__(
        ...
        segmentation_model_params["in_channels"] = (
            n_img_channels // self.n_sample_in_scene
        )
        self.sg_model = _get_segmentation_model("unet", segmentation_model_params)
