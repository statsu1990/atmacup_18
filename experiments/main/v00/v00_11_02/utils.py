...
class BackBone(nn.Module):
    def __init__(
        ...
        self.trained_sg_model = TrainedSegmentationModel()
        self.pre_sg_stem = nn.Sequential(
            nn.Conv2d(self.trained_sg_model.last_channels, 3, kernel_size=1),
            nn.BatchNorm2d(3),
        )
        self.sg_stem = nn.Sequential(
            nn.Conv2d(self.trained_sg_model.last_channels, 3, kernel_size=1),
            nn.BatchNorm2d(3),
        )
        self.next_sg_stem = nn.Sequential(
            nn.Conv2d(self.trained_sg_model.last_channels, 3, kernel_size=1),
            nn.BatchNorm2d(3),
        )
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(n_img_channels // self.n_sample_in_scene, 3, kernel_size=1),
            nn.BatchNorm2d(3),
        )
    ...
    def forward(
        ...
        # trained segmentation model
        # 学習済みモデルと正規化が異なるが無視する
        h_seg = 0
        with torch.no_grad():
            h_seg = self.trained_sg_model(x[:, 0:3])
        h_seg = self.pre_sg_stem(h_seg)
        with torch.no_grad():
            h_seg = self.trained_sg_model(x[:, 3:6])
        h_seg = self.sg_stem(h_seg)
        with torch.no_grad():
            h_seg = self.trained_sg_model(x[:, 6:9])
        h_seg = self.next_sg_stem(h_seg)
...
class TrainedSegmentationModel(nn.Module):
    def __init__(self):
        super().__init__()
        # encoder
        ...
        self.model = self._delete_classification_head(self.model, model_name)
        self.last_channels = self._get_last_channels(model_name)

    def _delete_classification_head(self, model: nn.Module, model_name: str):
    ...
    def _get_last_channels(self, model_name: str):
        if model_name == "deeplabv3_resnet50":
            return 256
        else:
            raise ValueError(f"model_name {model_name} is not supported.")
