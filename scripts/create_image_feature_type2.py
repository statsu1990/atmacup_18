...
def _generate_feature_for_batch(
    ...
    for image_name, image in images.items():
        ...
        with torch.no_grad():
            ...
            # (batch, token, feature)
            # tokens=257 (おそらく1個のcls_token+256個のpixel_token)
            outputs = outputs.last_hidden_state
            n_channel = outputs.size(-1)

            # パッチ内で特徴量を平均
            h, w = 16, 16
            n_patch_h, n_patch_w = 4, 4
            patch_h, patch_w = h // n_patch_h, w // n_patch_w
            # (batch, n_patch_h, patch_h, n_patch_w, patch_w, feature)
            ...
            # (batch, n_patch_h * n_patch_w, patch_h, patch_w, feature)
            outputs = outputs.permute(0, 1, 3, 2, 4, 5).reshape(
                outputs.size(0),
                n_patch_h * n_patch_w,
                patch_h,
                patch_w,
            ...
            # (batch, n_patch_h * n_patch_w, feature)
            outputs = outputs.mean(dim=(2, 3))
            # (batch, n_patch_h * n_patch_w * feature)
            outputs = outputs.reshape(outputs.size(0), -1)
            if outputs.size(1) != n_channel * n_patch_h * n_patch_w:
                raise ValueError(
                    f"Feature size mismatched: {outputs.size(1)} != {n_channel * n_patch_h * n_patch_w}"
                )
...
if __name__ == "__main__":
    ...
    save_tr_csv = constants.TR_IMAGE_FEATURE_TYPE2_CSV
    save_ts_csv = constants.TS_IMAGE_FEATURE_TYPE2_CSV
