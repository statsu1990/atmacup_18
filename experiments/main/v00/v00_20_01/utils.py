...
def _get_mixup(alpha: float, n: int, dev: str):
    beta_dist = torch.distributions.Beta(alpha, alpha)
    mix_rates = beta_dist.sample((n,)).to(dev)
    mix_rates = 0.5 + (mix_rates - 0.5).abs()

    mix_idices = torch.randperm(n).to(dev)
    return mix_rates, mix_idices
...
class Trainer:
    ...
    def run_epoch(
        ...
        Args:
            ...
            epoch_rate (float): (epoch - 1) / max_epochs. 0.0 ~ 1.0
        ...
        for data in tqdm(loader):
            ...
            with torch.set_grad_enabled(train):
                ...
                with torch.amp.autocast(
                    ...
                    if train:
                        mixup_alpha = self.mixup_alpha * (1.0 - epoch_rate)
                        if mixup_alpha > 0.0:
                            mix_rates, mix_idices = _get_mixup(
                                mixup_alpha, imgs.size(0), self.dev
                            )
                            imgs = (
                                mix_rates.view(-1, 1, 1, 1) * imgs
                                + (1 - mix_rates.view(-1, 1, 1, 1)) * imgs[mix_idices]
                            )
                            features = (
                                mix_rates.view(-1, 1) * features
                                + (1.0 - mix_rates.view(-1, 1)) * features[mix_idices]
                            )
                            relative_scene_dsecs = (
                                mix_rates.view(-1, 1) * relative_scene_dsecs
                                + (1.0 - mix_rates.view(-1, 1))
                                * relative_scene_dsecs[mix_idices]
                            )
                            targets = (
                                mix_rates.view(-1, 1) * targets
                                + (1.0 - mix_rates.view(-1, 1)) * targets[mix_idices]
                            )
