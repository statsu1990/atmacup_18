...
class DNN(nn.Module):
    def __init__(
        ...
        self.head = Head(
            ...
            in_features=self.backbone.last_channels,
...
class BackBone(nn.Module):
    def __init__(
        ...
        # rgb image encoder
        rgb_img_encoder_name = "facebook/dinov2-small"
        self.rgb_img_encoder = AutoModel.from_pretrained(rgb_img_encoder_name)
        rgb_img_enc_image_size = 224
        self.resize_for_rgb_img_encoder = nn.Upsample(
            size=(rgb_img_enc_image_size, rgb_img_enc_image_size),
            mode="bilinear",
            align_corners=True,
        )
        rgb_img_enc_normalize_mean = (
            torch.tensor([0.485, 0.456, 0.406]).unsqueeze(-1).unsqueeze(-1)
        )
        rgb_img_enc_normalize_std = (
            torch.tensor([0.229, 0.224, 0.225]).unsqueeze(-1).unsqueeze(-1)
        )
        self.register_buffer("rgb_img_enc_normalize_mean", rgb_img_enc_normalize_mean)
        self.register_buffer("rgb_img_enc_normalize_std", rgb_img_enc_normalize_std)
        rgb_encoder_last_channels = 384
        ...
        self.image_encoder = nn.Sequential(
            # (64, 128) -> (32, 32)
            nn.Conv2d(
                n_img_channels // self.n_sample_in_scene, 32, kernel_size=3, padding=1
            ),
            nn.BatchNorm2d(32),
            nn.GELU(),
            nn.Conv2d(32, 32, kernel_size=(2, 4), padding=0, stride=(2, 4)),
            nn.BatchNorm2d(32),
            nn.GELU(),
            # (32, 32) -> (16, 16)
        ...
        self.ft_fc = nn.Sequential(
            nn.Linear(n_features // self.n_sample_in_scene + 1, embed_dim),
        )

        # agg
        self.ft_agg_fc = nn.Sequential(
            nn.GELU(),
            nn.Linear(n_sample_in_scene * embed_dim, embed_dim),
            nn.LayerNorm(embed_dim),
            nn.GELU(),
            nn.Dropout(dropout),
        )
        self.rgb_img_agg_fc = nn.Sequential(
            nn.GELU(),
            nn.Linear(
                n_sample_in_scene * self.n_img_in_id * rgb_encoder_last_channels,
                embed_dim,
            ),
            # nn.LayerNorm(embed_dim),
            # nn.GELU(),
    ...
    def forward(
        ...
        # rgb encoder
        # (batch * n_sample_in_scene, n_img_in_id * rgb, h, w)
        rgb_h = x[:, : self.n_img_in_id * self.n_rgb]
        # (batch * n_sample_in_scene * n_img_in_id, rgb, h, w)
        rgb_h = rgb_h.reshape(-1, self.n_rgb, x.size(2), x.size(3))
        # (batch * n_sample_in_scene * n_img_in_id, tokens, channels)
        rgb_h = self._encode_rgb_images(rgb_h)

        # image encoder
        # (batch * n_sample_in_scene, tokens, channels)
        h_img = self._encode_images(x)
        ...
        # agg rgb
        # (batch, tokens, n_sample_in_scene * n_img_in_id * channels)
        rgb_h = (
            rgb_h.view(n_batch, -1, rgb_h.size(-2), rgb_h.size(-1))
            .permute(0, 2, 1, 3)
            .reshape(n_batch, rgb_h.size(-2), -1)
        )
        # (batch, tokens, channels)
        rgb_h = self.rgb_img_agg_fc(rgb_h)

        # agg image
        # (batch, tokens, n_sample_in_scene * channels)
        h_img = (
            h_img.view(n_batch, -1, h_img.size(-2), h_img.size(-1))
            .permute(0, 2, 1, 3)
            .reshape(n_batch, h_img.size(-2), -1)
        )
        # (batch, tokens, channels)
        h_img = self.img_agg_fc(h_img)

        # concat
        # (batch, tokens, channels)
        h = torch.cat([h_img, rgb_h], dim=1)

        # agg feature
        # (batch, channels)
        h_feat = self.ft_agg_fc(h_feat.view(n_batch, -1))

        return h, h_feat

    def _encode_rgb_images(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x (torch.Tensor): (batch, channels, h, w)
        """
        h = x / 255.0
        h = (h - self.rgb_img_enc_normalize_mean) / self.rgb_img_enc_normalize_std

        with torch.no_grad():
            h = self.resize_for_rgb_img_encoder(h)
            # (batch, tokens, channels)
            self.rgb_img_encoder.eval()
            h = self.rgb_img_encoder(pixel_values=h).last_hidden_state
    ...
    def _encode_images(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        # (batch, channel, h, w)
        h = self.img_normalize(x)
        # (batch, channel, h, w)
        h = self.image_encoder(h)

        # (batch, h * w, channel)
        h = h.permute(0, 2, 3, 1).reshape(h.size(0), -1, h.size(1))
...
class Head(nn.Module):
    ...
    def forward(self, x: torch.Tensor, x_feat: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x (torch.Tensor): (batch, tokens, channels)
